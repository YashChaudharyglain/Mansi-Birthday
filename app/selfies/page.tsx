...
export default async function SelfiesPage() {
  const selfies = await prisma.selfie.findMany({ orderBy: { createdAt: 'desc' }, take: 1 })
  return (
      ...
      {selfies.length === 0 ? (
        ...
        <div className="max-w-4xl mx-auto">
          {selfies.map((s: any) => {
            ...
            return (
              <div key={s.id} className="rounded-2xl overflow-hidden border border-amber-200 shadow bg-white">
                <div className="bg-black">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={s.url} alt={`Selfie ${s.id}`} className="w-full h-[70vh] object-contain" />
                </div>
